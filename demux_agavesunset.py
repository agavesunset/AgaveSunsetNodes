    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "select": ("INT", { "default": 1, "min": 1, "max": 8, "step": 1, "display": "number" })
            }
        }

    # Eight wildcard outputs and one INT output (selected index)
    RETURN_TYPES = (WILDCARD,)*8 + ("INT",)
    RETURN_NAMES = tuple(f"out{i}" for i in range(1, 9)) + ("selected_index",)
    FUNCTION = "demux"
    CATEGORY = "nodes_AgaveSunset"

    def demux(self, input, select):
        # Ensure select is an int in [1,8]
        sel = int(select)
        if sel < 1 or sel > 8:
            raise ValueError(f"[demux_AgaveSunset] 'select' must be between 1 and 8 (got {sel})")
        ...
        # Prepare results: all 8 outputs default to blocker
        outputs = [blocker] * 8
